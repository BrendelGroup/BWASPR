...
#'                         threshold=0.25,qvalue=0.25,mc.cores=4,
...
show_dmsg <- function(mrobj,dmgenes.file,min.nsites=2,query.genes=''){
    ...
    #
    dmgenes <- read.table(dmgenes.file,header=TRUE,fill=TRUE,sep='\t')
    sample_match_list <- as.list(unique(as.character(dmgenes[,'comparison'])))
    ...
    lapply(sample_match_list, function(sample_match){
        ...
        site_dmg <- cbind(sub_pair_p_meth,sub_dmgenes)
        site_dmg <- site_dmg[,c(sample1,sample2,
                                'seqnames','start','end','width','strand',
                                'gene_Dbxref','gene_width','gene_ID')]
# Note: the following allows subset display, but only subsets of dmg
#	
        if (query.genes != "") {
...
# Note. We need to make sure that there are difference to show in the heatmap:
                if (max(abs(plot[,1]-plot[,2])) > 0) {
